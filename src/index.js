...
function loadMessages() {
  ...
  const recentMessagesQuery = query(collection(getFirestore(), 'messages'), orderBy('timestamp', 'desc'), limit(1112));
...
async function insertCities (){
  ...
  const citiesRef = await collection(getFirestore() , "cities");
  ...
  try {
    ...
    console.error('Error writing new cities to Firebase Database', error);
  }
...
async function showListCities(){
     ...
     console.log('List cities show querySnapshot:');
    const querySnapshot = await  getDocs(collection(getFirestore(), "cities"));
      querySnapshot.forEach((doc) => {
       ...
       // const listCities = query(collection(getFirestore(), 'cities'), orderBy('name', 'desc'), limit(12));
        console.log('Cities is capital: ');
       const q = query(collection(getFirestore(), "cities"), where("capital", "==", true));

      const queryCities = await getDocs(q);
      queryCities.forEach((doc) => {
        // doc.data() is never undefined for query doc snapshots
        console.log(doc.id, " => ", doc.data());
      });

      console.log(' Get city by ID {SF} : ');
